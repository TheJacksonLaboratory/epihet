...
\section{Background}
  ...
  by the user.
    ...
    proportion of discordant reads (PDR). PDR at each locus is defined as a
    ...
    locus, a concordant read is one that shows all unmethlated or all methylated sites at a given loci, such as a four methylated cytosines. A discordant
    ...
    \\In this package, epihet calculates PDR value for each locus of one sample from the percentages of each methylation pattern.
    ...
    added together and subtracted from 1. The resulting value is the epipolymorphism value for the given locus.
...
@
  ...
  The parameter 'p' can range from 0 to 1. The comparison matrix comp.Matrix contains epigenetic heterogeneity values of the locus shared by the 100p percentile samples. For example, the loci in comp.Matrix are shared by at least half of the samples, then 'p' should be set to 0.50.
