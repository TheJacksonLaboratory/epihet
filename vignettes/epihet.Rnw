...
one time. Firstly, the customized metrics on all the loci for one sample are saved as an elements of a list.
The example data 'myValues' provided by epihet. Here we denoted the customized metrics as myValues.
Then, we added the myValues into the metadata of Granges object from GR.List sample by sample.
...
is generated, the users can also perform other downstream analysis using the customized metircs
