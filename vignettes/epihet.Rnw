...
output from \href{https://github.com/ShengLi/methclone}{methclone}, a C++ library
that analyzes the evolution of epialleles using Bisulfite Sequencing methylation data.
...
\section{Building the Comparison Matrix}
  ...
  samples from the program methclone and transforms the data into a large matrix that contains the location information and PDR, epipoly, and Shannon entropy
