...
can be used to calculate intra-tumoral epigenetic heterogeneity and analyze the
results through various types of graphs. This package was designed to use
output from \href{https://github.com/TheJacksonLaboratory/Methclone}{methclone}, a C++ library
that analyzes the evolution of epialleles using Bisulfite Sequencing.

Load this package into R session.
<<>>=
library(EpiHet)
@
...
\section{Background}
  ...
  myeloid leukemia (AML)(Li et al., 2016), chronic lymphocytic leukemia (CLL)(Landau et al., 2014),
  diffuse large B\-cell lymphoma, and Ewing sarcoma (Sheffield et al, 2017). 
  As the cancer cell evolves from diagnosis to relapse, methylation patterns at a given loci
  ...
  (Li et al, 2014). This package uses epialleles consisting of adjacent four cytosine
  ...
  \subsection{Bisulfite Sequencing}
  To determine the methylation patterns of cancer patients, a DNA sample
  ...
  heterogeneity between the samples can be analyzed by the user.
    ...
    total reads from that locus. the range of PDR values is \[0,1\]. 
    PDR values can then be used for analyzing
    epigenetic heterogeneity because a greater value for PDR corresponds
    to a greater amount of discord within the sample. With a greater level of discord, 
    the sample is considered to be more heterogeneous in nature and could lead to 
    adverse clinical outcomes upon treatment.
...
\section{Building the Comparison Matrix}
  ...
  samples from the tool methclone and transforms the data into a large matrix that contains the location information and PDR, epipoly, and Shannon entropy
  ...
  \href{https://github.com/ShengLi/methclone}{methclone}, which creates compressed text files containing epiallele patterns and the percentage of reads supporting each of epiallele patterns at the genomic locus in sample. The suffix of the compressed text files should be methClone_out.gz. Included in EpiHet are example files used for the sample code in this vignette, which include only the result for epialleles on chromosome 22 for two normal samples (N1, N2) and two AML patients with the CEBPA\_sil (isocitrate dehydrogenase 2) mutation (D2238, D2668). The following lines of code can be run to obtain the files and their corresponding ID names:
...
@
  ...
  To process the methClone_out.gz files, EpiHet has a function called makeGR which reads in the compressed files and creates a GenomicRanges
